...
public class UbershaderProvider implements MaterialProvider {
    ...
    public MaterialInstance createMaterialInstance(MaterialKey config,
            int[] uvmap, String label, String extras) {
        long nativeMaterialInstance = nCreateMaterialInstance(mNativeObject, config, uvmap, label, extras);
        return nativeMaterialInstance == 0 ? null : new MaterialInstance(mEngine, nativeMaterialInstance);
    }

    public Material getMaterial(MaterialKey config, int[] uvmap,
            String label) {
